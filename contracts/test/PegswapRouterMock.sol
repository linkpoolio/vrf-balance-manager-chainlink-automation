 ...
 */
contract PegSwap is Ownable, ReentrancyGuard {
    ...
    function addLiquidity(
        ...
        require(
            owner() == msg.sender || _hasLiquidity(source, target),
            "only owner can add pairs"
        );
    ...
    function swap(
        ...
        require(
            IERC20(source).transferFrom(msg.sender, address(this), amount),
            "transferFrom failed"
        );
        require(IERC20(target).transfer(msg.sender, amount), "transfer failed");
    }
     ...
     */
    function recoverStuckTokens(uint256 amount, address target)
        external
        onlyOwner
    {
    ...
    function onTokenTransfer(
        ...
        require(IERC20(target).transfer(sender, amount), "transfer failed");
    }
    ...
    function _addLiquidity(
        ...
        uint256 newAmount = getSwappableAmount(source, target) + amount;
    ...
    function _hasLiquidity(address source, address target)
        private
